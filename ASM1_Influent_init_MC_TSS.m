...
% % 
n = 2000 ; % number of latin hypercube samples
p = 4; % number of variables
X = lhsdesign(n,p,'smooth','off');

%% model uncertainty (5%)
p1 = [0.75*80 1.25*80];             %% CODpart
p2 = [0.75*0.4 1.25*0.4];             %% FFfraction
p3 = [0.75*700 1.25*700];                  %% M_Max
p4 = [0.75*10000 1.25*10000];                 %% Q_lim
...
CODpart_gperPEperd=RandomMatrix1(a,1);                % Particulate COD load in g COD/d per PE
...
factor1 = 2;                              % Proportionality factor random noise generators
...
FFfraction = RandomMatrix1(a,2);                          % Fraction of the TSS that is able to settle in the sewers

% ASM1 X state values
ASM1_XINIT=[0.25 0.1 0.1 0.1 0.0 0.0 0.02];% Initital conditions (7 states) XINIT= [TSS  XI  XS  XBH  XBA  XP  XND]

% Parameters
M_Max = RandomMatrix1(a,3);                               % kg SS
Q_lim = RandomMatrix1(a,4);                              % m3/d
n     = 15;                                 % Dimensionless
...
EvaluationMatrix_Temp = zeros(1000,12);

EvaluationMatrix_Temp (a,1)= mean(ASM1_Influentpart(:,15));
EvaluationMatrix_Temp (a,2)= std(ASM1_Influentpart(:,15));
EvaluationMatrix_Temp (a,3)= (EvaluationMatrix_Temp (a,1)./EvaluationMatrix_Temp (a,2));
EvaluationMatrix_Temp (a,4)= skewness(ASM1_Influentpart(:,15));
EvaluationMatrix_Temp (a,5)= mode(ASM1_Influentpart(:,15));
EvaluationMatrix_Temp (a,6)= kurtosis(ASM1_Influentpart(:,15));
EvaluationMatrix_Temp (a,7)= prctile(ASM1_Influentpart(:,15),95);
EvaluationMatrix_Temp (a,8)= prctile(ASM1_Influentpart(:,15),90);
EvaluationMatrix_Temp (a,9)= prctile(ASM1_Influentpart(:,15),85);

EvaluationMatrix_Temp (a,10)= prctile(ASM1_Influentpart(:,15),5);
EvaluationMatrix_Temp (a,11)= prctile(ASM1_Influentpart(:,15),10);
EvaluationMatrix_Temp (a,12)= prctile(ASM1_Influentpart(:,15),15);

y1(:,a)=ASM1_Influentpart(:,15);

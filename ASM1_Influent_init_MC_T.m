...
%% model uncertainty (5%)
p1 = [0.95*5 1.05*5];             %% TAmp
p2 = [0.95*18 1.05*18];             %% TBias
p3 = [0.95*0.6 1.05*0.6];                  %% TdAmp
% p4 = [0.75*0.01 1.25*0.01];                 %% TdBias
...
TAmp = RandomMatrix1(a,1);                                   % Sine wave amplitude (deg. C)
TBias = RandomMatrix1(a,2);                                 % Sine wave bias (m3/d) (= average infiltration flow rate)
TFreq = 2*pi/364;                           % Sine wave frequency (rad/d)
TPhase = pi;                         % Sine wave phase shift

TdAmp = RandomMatrix1(a,3);                                % Sine wave amplitude (deg. C)
...
EvaluationMatrix_Temp (a,1)= mean(ASM1_Influentpart(:,17));
EvaluationMatrix_Temp (a,2)= std(ASM1_Influentpart(:,17));
EvaluationMatrix_Temp (a,3)= (EvaluationMatrix_Temp (a,1)./EvaluationMatrix_Temp (a,2));
EvaluationMatrix_Temp (a,4)= skewness(ASM1_Influentpart(:,17));
EvaluationMatrix_Temp (a,5)= mode(ASM1_Influentpart(:,17));
EvaluationMatrix_Temp (a,6)= kurtosis(ASM1_Influentpart(:,17));
EvaluationMatrix_Temp (a,7)= prctile(ASM1_Influentpart(:,17),95);
EvaluationMatrix_Temp (a,8)= prctile(ASM1_Influentpart(:,17),90);
EvaluationMatrix_Temp (a,9)= prctile(ASM1_Influentpart(:,17),85);

EvaluationMatrix_Temp (a,10)= prctile(ASM1_Influentpart(:,17),5);
EvaluationMatrix_Temp (a,11)= prctile(ASM1_Influentpart(:,17),10);
EvaluationMatrix_Temp (a,12)= prctile(ASM1_Influentpart(:,17),15);

y1(:,a)=ASM1_Influentpart(:,17);
